---
title: "Election Security Grants: Budget and Spending Analysis"
author: "Luke Maier"
date: '2022-04-18'
output: pdf_document
header-includes: 
...
```{r hava, echo=FALSE,message=FALSE,warning=F,results='asis'}
...
#add quartiles
epi18$epi_quartile_2018<-cut(epi18$epi_2018,quantile(epi18$epi_2018),na.rm=T,
                      include.lowest=TRUE,labels=FALSE)
epi18$epi_quartile_2019<-epi18$epi_quartile_2018

#graph 2020 epi data                            
epi20$epi_2020<-epi20$index
epi20$epi_2021<-epi20$index
epi20$epi_quartile_2020<-cut(epi20$epi_2020,quantile(epi20$epi_2020),na.rm=T,
                      include.lowest=TRUE,labels=FALSE)
epi20$epi_quartile_2021<-epi20$epi_quartile_2020

#combine 18 and 20 epi datasets and ransform to long format
controls<-epi18 %>% select(-index) %>% left_join(., epi20, by="state_abbv") %>% select(-index)%>%
  pivot_longer(
    cols = -state_abbv,
    names_to = c('Item', '.value'),
    names_pattern = '(.*)_(\\w+)') %>%
  pivot_longer(
    cols = -c(state_abbv,Item),
    names_to = "Year") %>%
...
#add covid deaths
covid<-read.csv("nyt_covid.csv") %>%
  mutate(Year=as.character(Year))

controls<-left_join(controls,covid,by=c("State","Year"))
controls$covid_deaths[is.na(controls$covid_deaths)]=0

#clean up workspace
remove(epi18,epi20,covid,parties,aspep,tot,b)
...
theme_update(
  ...
  plot.caption = element_text(hjust=0, face="italic",size=10, ),#plot.caption.position ="plot",
  
  plot.background = element_rect(color = "deepskyblue", size = 3, fill=NA), 
...
#graph annual spending per category
budgets_and_spending %>% group_by(Year,Category) %>% 
  ...
  scale_y_continuous(breaks = seq(0,900000000,10000000),labels = comma_format(big.mark = ",",
                                           decimal.mark = "."))+
  labs(title = "Annual Spending per Category", caption = str_wrap("Note: Includes federal and state matching spending. Spending in 2018 was low in all categories, so the 2018 and 2019 expenditures were combined to make the chart more legible.",85))+
...
budgets_and_spending %>% group_by(Year,Category) %>% 
  ...
  scale_y_continuous(breaks = seq(0,900000000,25000000),
  ...
  labs(title = "Culumative Spending Compared to Budget",
       caption = str_wrap("Note: The total amount states budgeted for each category is outlined in red. In 2020, states submitted a revised budget combining the 2018 and 2020 election security grants.",70))+
  theme(axis.text.x=element_text(angle=45,hjust = 1))
...
budgets_and_spending %>% group_by(Category) %>% 
  ...
  geom_flow(aes(alluvium = Category),alpha=.8,width=.3,curve_type = "cubic")+  
  
  geom_text(aes(label = paste0(round(Percent_Total_Expenditures*100),"%")), 
            color="black",
            position = position_stack(vjust = 0.5), size = 3)+
  
  labs(title = "Cumulative Expenditures Per Category",
       caption = str_wrap("Note: Includes federal and state matching spending. Spending in 2018 was low in all categories, so the 2018 and 2019 expenditures were combined to make the chart more legible.",85))+
...
budgets_categories %>% group_by(Category) %>% 
  ...
  labs(title = "Budget Distribution in 2018 vs 2020",
       caption = str_wrap("Note: The sum of the budgets per category does not equal the total grant funds provided by Congress because some states did not submit budgets accounting for all of their funding.",85))+
  scale_fill_calc()+
...
sums %>% filter(is.na(Year)==F)%>%
  ...
  geom_text(aes(label = paste0(round(`Percent Budget`*100),"%")), 
            color="black",
            position = position_stack(vjust = 0.5), size = 3)+
  labs(title = "Percent of Funds Budgeted Per Category", caption = str_wrap("Note: The 2020 columns reflect states' revised budgets that include both 2018 and 2020 grants.",85))+
  scale_fill_calc()+
...
sums %>% filter(is.na(Year)==T) %>% select(-Year)%>%
  ...
  labs(title = str_wrap("Percent of Federal Funds Budgeted by Category and Cost Type",40),
  caption = str_wrap("Note: The 2020 columns reflect states' revised budgets that include both 2018 and 2020 grants.",85))+
  scale_y_continuous(labels = scales::percent_format(scale =  100))
...
#calculate percent federal funds spent to date
...
#join federal and state spending with controls
...
dat %>% filter(State!="DE")%>%
  ...
  labs(title = "Cumulative State Matching Spent",
       caption = str_wrap("Note: The size of each state's dot corresponds to how much state matching funds it is required to provide. The shading of each state's dot indicates percentage of matching funds it spent. The slope of the red trend line shows the average percent change in cumulative spending from year to year. A logarithmic transformation was applied to the vertical access to make these trends in cumulative spending more legible by shrinking visual disparities between large and small states.",85))+
  ylab("Logged dollars")+
...
dat %>% 
  ...
  #add labels and gridlines
  labs(title = "Cumulative Federal Funds Spent",
       caption = str_wrap("Note: The size of each state's dots corresponds to the size of its federal grants. The shading of each state's dot indicates percentage of its federal funds it spent. The slope of the red trend line shows the average percent change in cumulative spending from year to year. A logarithmic transformation was applied to the vertical access to make these trends in cumulative spending more legible by shrinking visual disparities between large and small states.",85))+
  ylab("Logged dollars")+
  coord_cartesian(clip="off",expand = T)+
...
dat %>% filter(State!="DE") %>%
  ...
  scale_y_continuous(labels = scales::percent_format(scale = 100))+
  #add labels and gridlines
  labs(title = "Cumulative Percent of State Matching Funds Spent",
       caption = str_wrap("Note: The size of each state's dots corresponds to the size of its required matching amount. The darkness of the shading corresponds to the amount of matching funds it spent so far. The red line shows the average percent of state matching funds spent by the end of each fiscal year.",85))+
  ylab("Percent of State Matching Funds Spent so Far")+
  geom_vline(xintercept = c("2018.5","2019.5","2020.5"), 
             linetype="dashed",color="red",alpha=.2,size=.5)+
  coord_cartesian(clip="off",expand = T)+
  scale_y_continuous(labels = scales::percent_format(scale = 100))+
...
dat %>% 
  ...
  scale_y_continuous(labels = scales::percent_format(scale = 100))+
  #add labels and gridlines
  labs(title = "Cumulative Percent Federal Funds Spent",
       caption = str_wrap("Note: The size of each state's dots corresponds to the size of its federal election security grants. The darkness of the shading corresponds to the amount of these grants it spent so far. The red trend line shows the average percent of federal funds spent by the end of the fiscal year.",85))+
  ylab("Percent of Federal Funds Spent So Far")+
  geom_vline(xintercept = c("2018.5","2019.5","2020.5"), 
             linetype="dashed",color="red",alpha=.2,size=.5)+
  coord_cartesian(clip="off",expand = T)+
...
dat %>% filter(Party=="Republican"|Party=="Democrat")%>%
  ...
  labs(title="Percent State Matching Funds Spent by Governor's Party",
       caption = str_wrap("Note: The boxes show the average (the black line) and range for the 25th-75th percentile (the box) of states according to the percent of their required state matching funds spent to date. The dots show each state, sized by the amount of matching funds they're required to provide. Deleware was excluded because they were an outlier that spent far more state matching funds than required by HAVA.",85))+
  ylab("Percent of Required State Matching Spent So Far")

#t-test for significant diff in party spending
t<-dat %>% filter(Year=="2021") %>% 
  t_test(State_Pct_Matching_Spent ~ Party) %>%
  select(-c(".y.",group1,group2,n1,n2))
kable(t,caption = "Test for significant differences in Pecent State Matching Funds Spent by Governor's Party, 2021 (Two-sample t-test)")
pwr.t2n.test(n1=nrow(filter(dat,Year=="2021",Party=="Republican")),
             n2=nrow(filter(dat,Year=="2021",Party=="Democrat")),d=.5,sig.level=.10)
...
dat %>% filter(Party=="Republican"|Party=="Democrat")%>%
  ...
  labs(title="Percent Federal Funds Spent by Governor's Party",
       caption = str_wrap("Note: The boxes show the average (the black line) and range for the 25th-75th percentile (the box) of states according to the percent of federal funds spent so far. The dots shows each state, sized by their federal funds spent.",85))+
  ylab("Percent Federal Funds Spent So Far")

#t-test for significant diff in party spending
t<-dat %>% filter(Year=="2021") %>% 
  t_test(Federal_Pct_Spent ~ Party) %>%
  select(-c(".y.",group1,group2,n1,n2))
kable(t,caption = "Test for significant differences in Pecent Federal Funds Spent by Governor's Party, 2021 (Two-sample t-test)")
pwr.t2n.test(n1=nrow(filter(dat,Year=="2021",Party=="Republican")),
             n2=nrow(filter(dat,Year=="2021",Party=="Democrat")),d=.5,sig.level=.10)
...
budgets_and_spending %>%
  ...
  labs(title=str_wrap("Percent of Budgeted Funds Spent Through FY21 by Governor's Party",40),
       caption = str_wrap("Note: The boxes show the average (the black line) and range for the 25th-75th percentile (the box) of states according to the percent spent by the end of FY21 of their budgets for each category. Outliers with spending significantly different than their party average were hidden from the graph but included when defining the boxes and averages.",85))+
  theme(axis.text.x=element_text(angle = 45, hjust=1,vjust = 1 ))+
  ylab("Percent of Budget for Each Category Spent")

#little dataset of total state+federal budget
tot<-allocations %>% filter(Item=="Total_Fed_Plus_State_Funds") %>%
  select(-Item) %>%
  rename(Total_Budget=Amount)

#calculate percent of total budget  for each category
b<-budgets_and_spending %>%
  filter(Category!="Totals" & Category!="Other COVID")%>%
  left_join(.,controls,by=c("State","Year")) %>%
  filter(Party=="Republican"|Party=="Democrat")%>%
  left_join(.,tot,by=c("State","Year")) %>%
  mutate(Percent_Total_Funds_Spent= Cumulative_Expenditure/Total_Budget,
         Percent_Total_Budget= Budgeted/Total_Budget)

b %>%  
  filter(Year=="2020") %>%
  ggplot(aes(x=Category, y=Percent_Total_Budget*100,fill=factor(Party)))+
  geom_boxplot(aes(fill=factor(Party)),alpha=.6)+
  ...
  scale_y_continuous(labels = scales::percent_format(scale = 1))+
  labs(title=str_wrap("Percent of Funds Budgeted Per Category by Governor's Party",40),
       caption = str_wrap("Note: The boxes show the average (the black line) and range for the 25th-75th percentile (the box) of states according to the percent spent by the end of FY21 of their budgets for each category. Outliers with spending significantly different than their party average were hidden from the graph but included when defining the boxes and averages.",85))+
  theme(axis.text.x=element_text(angle = 45, hjust=1,vjust = 1 ))+
  ylab("Percent of Budget for Each Category Spent")

#t-test on differences in mean budget (2018+2020 combined) per category by party
t<-b %>% filter(Year=="2021") %>% 
  t_test(Percent_Total_Budget ~ Party) %>%
  select(-c(".y.",group1,group2,n1,n2))
kable(t,caption = "Test for significant differences in Pecent Funds Budgeted Per Category by Governor's Party (Two-sample t-test)")
pwr.t2n.test(n1=nrow(filter(dat,Year=="2021",Party=="Republican")),
             n2=nrow(filter(dat,Year=="2021",Party=="Democrat")),d=.5,sig.level=.10)


#t-test difference in mean cumulative spending by 2021 per category per party
t<-b %>% filter(Year=="2021") %>%  
  t_test(Percent_Total_Funds_Spent ~ Party)%>%
  select(-c(".y.",group1,group2,n1,n2))
kable(t,caption = "Test for significant differences in Pecent Funds Spent Per Category by Governor's Party (Two-sample t-test)")
pwr.t2n.test(n1=nrow(filter(dat,Year=="2021",Party=="Republican")),
             n2=nrow(filter(dat,Year=="2021",Party=="Democrat")),d=.5,sig.level=.10)

#t-test of percent state matching spent vs party
t<-dat %>% filter(Year=="2021" & State!="DE") %>%
  t_test(State_Pct_Matching_Spent ~ Party)  %>%
  select(-c(".y.",group1,group2,n1,n2))
kable(t,caption="Test for significant differences in State Matching Provided by Governor's Party  (Two-sample t-test)")
pwr.t2n.test(n1=nrow(filter(dat,Year=="2021",Party=="Republican")),
             n2=nrow(filter(dat,Year=="2021",Party=="Democrat")),d=.5,sig.level=.10)


a<-controls %>% group_by(Year) %>% summarize(Dems=sum(Party=="Democrat"),Reps=sum(Party=="Republican"))
kable(a)
remove(a)
#####
...
#graph spending per category per EPI quartile 
budgets_and_spending %>%
  filter(Year==2021 & Category!="Totals" & Category!="Other COVID")%>%
  left_join(.,controls,by=c("State","Year")) %>%
  filter(epi_quartile!=is.na(epi_quartile)) %>%
  left_join(.,tot,by=c("State","Year")) %>%
  mutate(Percent_Total_Funds_Spent= Cumulative_Expenditure/Total_Budget) %>%
  
  ggplot(aes(x=Category, y=Percent_Total_Funds_Spent*100,fill=factor(epi_quartile)))+
  geom_boxplot(aes(fill=factor(epi_quartile)),alpha=.8)+
  
  #geom_point(aes(group=factor(epi_quartile), size=Cumulative_Expenditure),show.legend=F, position=position_dodge(width=0.75),stat = "identity",alpha=.1)+
  ...
  scale_y_continuous(labels = scales::percent_format(scale = 1))+
  labs(title=str_wrap("Percent of Budget Spent Through FY21 by Election Performance Index Quartile",40),
       caption = str_wrap("Note: The box plots show the distribution of the percent of budgeted funds spent by the end of FY21. States were grouped by EPI quartile, with those in the lowest 25th percentile of EPIs grouped in blue and those with the highest 25th percentile of EPIs grouped in green. The black lines in the boxes show the average for each EPI quartile.",85))+
  theme(axis.text.x=element_text(angle = 45, hjust=1,vjust = 1 )) +
  ylab("Percent of Budget Spent through FY21")
...
dat %>% 
  ...
  labs(title = str_wrap("Election Performance Index vs. Cumulative Federal Funds Spent",40),
       caption = str_wrap("Note: A state's dot size corresponds to the size of its of federal grants. EPI data are only available for 2018 and 2020, so the EPI values for 2018 and 2020 were used for 2019 and 2021, respectively.",85))+
  ylab("Cumulative Federal Funds Spent (logged dollars)")+
...
dat %>% 
  ...
  labs(title = str_wrap("Election Performance Index vs. Percent Federal Funds Spent",40),
       caption = str_wrap("Note: A state's dot size corresponds to the size of its of federal grants. EPI data are only available for 2018 and 2020, so the EPI values for 2018 and 2020 were used for 2019 and 2021, respectively.",85))+
  ylab("Percent Federal Funds Spent")+
  xlab("Election Performance Index")+
  guides(size="none")

#pearson correlation coefficient between EPI and federal spending 
t<-dat %>% #filter(Year=="2021") %>% 
  cor_test(Federal_Pct_Spent, epi, method = "pearson") %>%
  select(-c(var1,var2,method))
kable(t, caption = "Correlation between EPI and Percent Total Federal Funds Spent (Pearson's pairwise correlation coefficient)")
pwr.r.test(n=nrow(b),r=t$cor,sig.level=.10)


#calculate percent of total budget and cumulative spending for each category
budgets_and_spending$Year[budgets_and_spending$Year=="2018-2019"]="2019"
b<-budgets_and_spending %>%
  filter(Category!="Totals" & Category!="Other COVID")%>%
  left_join(.,controls,by=c("State","Year")) %>%
  filter(Party=="Republican"|Party=="Democrat")%>%
...
#pearson correlation coefficient between EPI and spending per category
t<-b %>% #filter(Year=="2021") %>%
  cor_test(Percent_Total_Budget, epi, method = "pearson") %>%
  select(-c(var1,var2,method))
kable(t, caption = "Correlation between EPI and Percent Total Budgeted per Category (Pearson's pairwise correlation coefficient)")
pwr.r.test(n=nrow(b),r=t$cor,sig.level=.10)

t<-b %>% #filter(Year=="2021") %>% 
  cor_test(Percent_Total_Funds_Spent, epi, method = "pearson")  %>%
  select(-c(var1,var2,method))
kable(t, caption = "Correlation between EPI and Percent Total Spent per Category (Pearson's pairwise correlation coefficient)")
pwr.r.test(n=nrow(b),r=t$cor,sig.level=.10)

#autoregression (AR1) for EPI
e<-dat %>% group_by(State, Year) %>%
  filter(Year!="2018",Year!="2021") %>%
  select(State,Year,epi,Federal_Funds_Spent,covid_deaths) %>%
  pivot_wider(names_from = "Year",
              values_from = c("epi","Federal_Funds_Spent","covid_deaths"))
ar11<-lm(e$epi_2020 ~ e$epi_2019)
#see if adding federal funds spent improves AR model
ar12<-lm(e$epi_2020 ~ e$epi_2019 + log1p(e$Federal_Funds_Spent_2019))
ar13<-lm(e$epi_2020 ~ e$epi_2019 + log1p(e$Federal_Funds_Spent_2019) + log1p(e$covid_deaths_2020))

stargazer(ar11, ar12,ar13, title = "Autoregression Comparisons for EPI",
          keep.stat=c("n","ll","rsq"),
          header = FALSE, column.sep.width = ".25pt")
...
dat %>% 
  ...
  xlim(.6,.91)+
  scale_y_continuous(expand = c(0, 0),limits = c(0,max(log(dat$Federal_Funds_Spent))+3))+
  labs(title = str_wrap("Election Performance Index vs. State Matching Funds Spent", 40),
       caption = str_wrap("Note: A state's dot size corresponds to the size of its required state matching funds. EPI data are only available for 2018 and 2020, so the EPI values for 2018 and 2020 were used for 2019 and 2021, respectively.",85))+
  ylab("State Matching Funds Spent (logged dollars)")+
  xlab("Election Performance Index")+
  guides(size="none")
...
dat %>% filter(State!="DE")%>%
  ...
  labs(title = str_wrap("Election Performance Index vs. Percent State Matching Funds Spent",40),
       caption = str_wrap("Note: A state's dot size corresponds to the size of its required state matching funds. EPI data are only available for 2018 and 2020, so the EPI values for 2018 and 2020 were used for 2019 and 2021, respectively.",85))+
  ylab("Percent State Matching Funds Spent")+
  xlab("Election Performance Index")+
  guides(size="none")


#t-test percent state matching spent vs. epi
t<-dat %>% filter(State!="DE") %>%
  cor_test(State_Pct_Matching_Spent, epi) %>%
  select(-c(var1,var2,method))
kable(t, caption = "Correlation between Percent State Matching Spent and EPI (Pearson's pairwise correlation coefficient)")
pwr.r.test(n=nrow(b),r=t$cor,sig.level=.10)


#####
#Graphs - Comparisons to covid deaths in 2020
#####

#compare pct fed funds spent to covid deaths
b<-dat %>% select(State,Year,Federal_Funds_Spent,State_Matching_Spent,covid_deaths) %>%
  pivot_wider(names_from = Year, values_from = c(Federal_Funds_Spent, State_Matching_Spent, covid_deaths)) %>%
  mutate(Federal_Pct_Change_2020=log1p(Federal_Funds_Spent_2020)-log1p(Federal_Funds_Spent_2019),
         Federal_Pct_Change_2021=log1p(Federal_Funds_Spent_2021)-log1p(Federal_Funds_Spent_2020),
         State_Pct_Change_2020=log1p(State_Matching_Spent_2020)-log1p(State_Matching_Spent_2019),
         State_Pct_Change_2021=log1p(State_Matching_Spent_2021)-log1p(State_Matching_Spent_2020),
         covid_deaths_log_2020=log1p(covid_deaths_2020),
         covid_deaths_log_2021=log1p(covid_deaths_2021))
b<- b %>% 
  pivot_longer(
    cols = -State,
...
b %>% 
  ggplot(aes(x=covid_deaths_log,y=Federal_Pct_Change,color=Year, size=covid_deaths*2))+
  stat_smooth(group="none",
              method = "lm", formula = y ~ x , 
              size = .8, se=T, level=.9, color = "red", show.legend = F, alpha=.5)+
  geom_point(alpha=.5) +
  geom_text(aes(label=State, size=covid_deaths*.2),color="black",alpha=1)+
  
  scale_color_manual(values = c("red","orange"))+
  coord_cartesian(clip="off",expand = T)+
  scale_y_continuous(labels = scales::percent_format(scale = 1))+
  labs(title = str_wrap("COVID Deaths vs. Percent Change in Total Federal Funds Spent",40),
       caption = str_wrap("Note: A state's dot size corresponds to its number of COVID deaths.",85))+
  ylab("Percent Change in Cumulative Federal Funds Spent")+
  xlab("COVID Deaths (Logged)")+
  guides(size="none")


#test correlations between covid deaths and percent change in federal funds spent
t<-b %>% cor_test(covid_deaths_log,Federal_Pct_Change) %>%
  select(-c(var1,var2,method))
kable(t, caption="Correlation betwen Covid Deaths and Percent Change in Federal Spending (Pearson's pairwise correlation coefficient)")
pwr.r.test(n=nrow(b),r=t$cor,sig.level=.10)
t<-b %>% cor_test(covid_deaths_log,State_Pct_Change) %>%
  select(-c(var1,var2,method))
kable(t,caption="Correlation betwen COVID Deaths and Percent Change in State Spending (Pearson's pairwise correlation coefficient)")
pwr.r.test(n=nrow(b),r=t$cor,sig.level=.10)


#test correlation between covid deaths adn spending in each  category
b <-left_join(budgets_and_spending,controls, by=c("State","Year")) %>%
  filter(Category!="Totals" & Category!="Other COVID") %>% 
  select(c(Category, State, Year, Cumulative_Expenditure, covid_deaths)) %>% 
  pivot_wider(names_from = Year, values_from = c(Cumulative_Expenditure, covid_deaths)) %>%
  mutate(Category_Pct_Change_2020=log1p(Cumulative_Expenditure_2020)-log1p(Cumulative_Expenditure_2019),
         Category_Pct_Change_2021=log1p(Cumulative_Expenditure_2021)-log1p(Cumulative_Expenditure_2020),
         covid_deaths_2020_log=log1p(covid_deaths_2020),
         covid_deaths_2021_log=log1p(covid_deaths_2021))

t<-b %>% cor_test(covid_deaths_2020_log,Category_Pct_Change_2020)  %>%
  select(-c(var1,var2,method))
kable(t,caption = "Correlation between COVID Deaths and Percent Change in Spending per Category, 2020 (Pearson's pairwise correlation coefficient)")
pwr.r.test(n=nrow(b),r=t$cor,sig.level=.10)
t<-b %>% cor_test(covid_deaths_2021_log,Category_Pct_Change_2021)  %>%
  select(-c(var1,var2,method))
kable(t,caption = "Correlation between COVID Deaths and Percent Change in Spending per Category, 2021 (Pearson's pairwise correlation coefficient)")
pwr.r.test(n=nrow(b),r=t$cor,sig.level=.10)
